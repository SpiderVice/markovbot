...
handler = logger.add("logs/discord.log")

def try_load_model() -> markovify.NewlineText:
    if not os.path.exists("data/markov_model.json") or model_manager.should_rebuild_model():
        logger.info(
            f"markov_model.json not found or STATE_SIZE has changed. Loading messages.txt and creating model...")
        text_model: markovify.NewlineText = model_manager.build_markov_model()
        logger.info(
            f"Saving model to markov_model.json...")
        # model_manager.save_model(botconfig.STATE_SIZE)
        return text_model
    else:
        logger.info(
            f"markov_model.json found with matching STATE_SIZE. Loading model...")
...
def random_with_lookup(look_up_term: str) -> str:
    final_message = ""
    logger.info(
        f"Generating random message with lookup term: {look_up_term}")
    tries = 0
